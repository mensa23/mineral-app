...
function recoverWallet(): Ed25519Keypair | null {
    const val = sessionStorage.getItem(WALLET_KEY);
    if (!val) {
        return null;
    }
...
function recoverMiningProgress(): { nonce: bigint; hash: Uint8Array } | null {
    const val = sessionStorage.getItem(MINE_KEY);
    if (!val) {
        return null;
    }
...
(async () => {
    ...
    app.ports.importWallet.subscribe((privateKey) =>
        (async () => {
            ...
            sessionStorage.setItem(WALLET_KEY, kp.getSecretKey());
            wallet = kp;
    ...
    app.ports.submitProof.subscribe(({proof, miner}) =>
        (async () => {
            ...
            sessionStorage.removeItem(MINE_KEY);
